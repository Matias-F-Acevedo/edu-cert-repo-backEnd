...
export class CertificateService {
  ...
  async generateCertificateRegular(studentId: number, examId: number) {
    try {
      // 1. Obtener los datos del estudiante desde la base de datos
      const student = await this.studentRepository.findOne({
        where: { id: studentId },relations:["career"]
      });
      ...
      // 2. Cargar la plantilla PDF
      const templatePath =
        'C:/Users/admin/Desktop/edu-cert-repo-backEnd/src/templates/template-regular-student.pdf'; // Ruta a tu plantilla PDF
      const existingPdfBytes = await fsPromises.readFile(templatePath);
      const pdfDoc = await PDFDocument.load(existingPdfBytes);
      ...
      const additionalText = `Se deja constancia que a la fecha, ${today.getDate()} de ${getMonthName(today.getMonth() + 1)} del año ${today.getFullYear()},
${student.lastName.toLocaleUpperCase()}, ${student.firstName.toLocaleUpperCase()} D.N.I.: ${formattedDNI}
es alumno/a regular del Instituto Superior del Sudeste DIEGEP Nº 6.400 de la carrera
${student.career.name.toLocaleUpperCase()}
Resolución No 2066/18
A pedido del interesado/a y para ser presentado ante quien corresponda,  
se extiende la presente en la ciudad de Benito Juárez.
`;
      // Función para obtener el nombre del mes a partir del número de mes
      function getMonthName(monthNumber: number): string {
        const months = [
